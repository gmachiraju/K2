...
from metrics import confusion_matrix, msd, auroc, auprc, ap
...
def train_gridsearch(sweep_dict, save_dir, encoder_name, gt_dir, process_args, model_args):
    ...
    """
    results_dict, results_dir, proc_cache_dir, model_cache_dir = setup_gridsearch(save_dir, encoder_name)
    ...
    for k in sweep_dict["k"]:
        ...
        for r in sweep_dict["r"]:
            ...
            for lam in sweep_dict["lam"]:
                ...
                model_results_dict = gridsearch_iteration(model, model_args, gt_dir)
...
def eval_suite(G_name, P, Y, y, y_hat, thresholds):
    ...
    for thresh in thresholds:
        ...
        datum_thresh_cm_dict[(G_name, thresh)] = confusion_matrix(P_bin, Y_vec)

    return datum_thresh_msd_dict, datum_thresh_cm_dict, datum_cont_dict, datum_pred_dict
...
def gridsearch_iteration(model, model_args, gt_dir, thresh="all"):
    ...
    for G_name in os.listdir(model_args["train_graph_path"]):
        G = deserialize(os.path.join(model_args["train_graph_path"], G_name))
        Y = deserialize(os.path.join(gt_dir, G_name)) # groud truth
        P = model.prospect(G)
        if thresh == "all":
        ...
        if model.classifier is not None and model.variant == "predictive":
...
def setup_gridsearch(save_dir, encoder_name):
    ...
    return results_dict, results_dir, proc_cache_dir, model_cache_dir
